...
func (h *StockHandler) AddStock(c *gin.Context) {
	...
	if err := h.stockService.AddStock(c.Request.Context(), req.ProductID, req.SubVariantID, req.Quantity); err != nil {
...
func (h *StockHandler) RemoveStock(c *gin.Context) {
	...
	if err := h.stockService.RemoveStock(c.Request.Context(), req.ProductID, req.SubVariantID, req.Quantity); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
